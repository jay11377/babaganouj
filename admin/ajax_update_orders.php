...
if($date_fin_7>$date_fin) // date a plus d'une semaine, on ne montre pas les commandes futures
	...
	if($date_debut==$date_fin) // Si une seule date délectionnée à moins d'une semaine, on montre toutes les commandes futures
		$query.= "AND ((C.date_livraison='".$date_debut."') OR (C.date_livraison>'".$date_debut."' AND C.id_statut=2)) ";
	else // Si intervalle de date à moins d'une semaine, on montre tous les commandes livrée dans l'intervalle de date, mais on masque les commandes futures à ces dates
