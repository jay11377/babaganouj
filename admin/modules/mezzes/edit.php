...
<div class="box">
	<div class="header">
    	<h3><?php showLang('EDIT_MEZZES') ?></h3>
    </div>
    <div class="container"><?php	
		...
		if(isset($_POST['options'])) 
			...
			$query="SELECT * FROM $module WHERE id_plat=".$id;	
			$result_options=$connector->query($query);
			while ($row_options = $connector->fetchArray($result_options))
				$options[] = $row_options['id_option'];
		}
		...
		if(isset($_POST['submit']) && $_POST['submit']==getLang('SUBMIT')){
			...
			if( isset($_FILES['photo1']) && $_FILES['photo1']['name'] != '' ){
				$imagepath = $validator->validateUploadImage('photo1',getLang('NO_IMAGE_GIVEN'),"plats",0,0);
				// Thumnail
				if($imagepath!=false){
					/* For the web */
			...
			if ( $validator->foundErrors() ){
				...
				$query_delete = "DELETE FROM $module WHERE id_plat = ".(int)$id;
				$result2 = $connector->query($query_delete);
				...
				if ($result && $result2){
						$_SESSION['pagemsg']='<div class="msg msg-ok"><p>'.getLang('MEZZES_UPDATED').'</p></div>';
