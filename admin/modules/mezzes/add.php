...
<div class="box">
	<div class="header">
    	<h3><?php showLang('ADD_MEZZES') ?></h3>
    </div>
    <div class="container"><?php	
		$connector = new DbConnector();
		$result_cat = $connector->query("SELECT * FROM categories WHERE menu=1 ORDER BY name"); /* Get the list of categories for the select option form */
		$result3 = $connector->query("SELECT * FROM tva ORDER BY value"); /* Get the list of tax rates */
		
		$result_options = $connector->query('SELECT * from options');
		...
		// Validate the entries
			...
			//Image
			$imagepath = $validator->validateUploadImage('photo1',getLang('NO_IMAGE_GIVEN'),"plats");
			...
			if( !($validator->foundErrors()) && $imagepath!=false){
				...
				$thumb->save($thumb_path,80); // save the thumbnail
				$thumb_dbpath=$thumb->getThumbPathDB($thumb_path); // set up the path for the database
				
				$thumb_details = new Thumbnail('../'.$imagepath);
				$thumb_details->resizeForCrop(67,50);
				$thumb_details->cropFromCenterSize(67,50);
			...
			if(!($validator->foundErrors()) && isset($_FILES['photo2']) && $_FILES['photo2']['name'] != '' ){
				...
				//photo2
				$image2path = $validator->validateUploadImage('photo2',getLang('NO_IMAGE_GIVEN'),"plats");
				...
				if($image2path!=false){
					...
					$thumb2_details->resizeForCrop(67,50);
					$thumb2_details->cropFromCenterSize(67,50);
			...
			if(!($validator->foundErrors()) && isset($_FILES['photo3']) && $_FILES['photo3']['name'] != '' ){
				...
				//photo3
				$image3path = $validator->validateUploadImage('photo3',getLang('NO_IMAGE_GIVEN'),"plats");
				...
				if($image3path!=false){
					...
					$thumb3_details->resizeForCrop(67,50);
					$thumb3_details->cropFromCenterSize(67,50);
			...
			if ( $validator->foundErrors() ){
				...
				$result_pos = $connector->query("SELECT MAX(order_position) AS position FROM plats where id_categorie = $id_categorie");
				...
				// Insert the dish in table plats
				$query = "INSERT INTO plats (name, ingredient_principal, description, menu, vegetarien, epice, id_tva, bigphoto1, photo1, photo_mobile1, thumbnail1, thumbnail_details1, thumbnail_mobile1, prix_ht, prix_ttc,id_categorie , order_position, active";
				...
				/*inserting data in mezzes table*/
				$sortedlist=explode(',',$_POST['sortedlist']);
				$id_plat = mysql_insert_id();
				for ($i = 0; $i < count($sortedlist); $i++) {
				  $j=$i+1;
				  $query = "INSERT INTO $module (id_plat, id_option, order_position, active) VALUES(
						".(int)$id_plat.",
						".(int)$sortedlist[$i].",
						".(int)$j.",
						1)";
