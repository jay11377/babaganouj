<div class="inset container">
    <form method="post" name="form" action="moduleinterface.php" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="required  field ">
            	<label for="id_plat"><?php showLang('DISH') ?></label>
                <select id="id_plat" name="id_plat">
                	<?php while($row_dish = $connector->fetchArray($result_plat)){ ?>
                    <option value="<?php echo $row_dish['id']?>" <?php if($row_dish["id"] == $id_plat) echo 'selected="selected"'?> >
                    	<?php echo $row_dish['name'] ?>
                    </option>
					<?php } ?>
                </select>
            </div>
