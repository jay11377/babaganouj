<?php
	$result = $connector->query("SELECT * FROM tva WHERE id=".(int)$id_tva);
    ...
    <form method="post" name="form" action="moduleinterface.php" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="field">
                ...
                <input type="checkbox" id="vegetarien" name="vegetarien" <?php if($vegetarien==1) echo 'checked="checked"' ?> />
            </div>
            
            <div class="field">
            	<label><?php showLang('SPICY') ?></label>
                <input type="checkbox" id="epice" name="epice" <?php if($epice==1) echo 'checked="checked"' ?> />
            </div>
             ...
             <div class="required field ">
            	<label for="id_categorie"><?php showLang('CATEGORY') ?></label>
                	<select name="id_categorie" id="id_categorie">
                    	...
                    	<option value="<?php echo $row['id']?>" <?php if($row['id'] == $id_categorie) echo'selected="selected"';?> ><?php echo $row['name']?></option>
                        <?php } ?>
                    </select>
            </div>
