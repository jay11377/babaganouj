...
<div class="box">
	<div class="header">
    	<h3><?php showLang('ADD_OPTION') ?></h3>
    </div>
    <div class="container"><?php	
		$connector = new DbConnector();
		$query="SELECT * FROM plats"; /* to display the list of dishes in the select list option form */
		$result_plats = $connector->query($query);
		$name = (isset($_POST[ 'name' ]))?$_POST[ 'name' ]: ''; 
		$name_admin = (isset($_POST[ 'name_admin' ]))?$_POST[ 'name_admin' ]: ''; 
		...
		// Validate the entries
			...
			if ( $validator->foundErrors() ){
				...
				// Insertion
				$query = "INSERT INTO options (name, name_admin) VALUES (".
				"'".isql($name)."',".
				"'".isql($name_admin)."')";

				if ($result = $connector->query($query)){
					$_SESSION['pagemsg']='<div class="msg msg-ok"><p>'.getLang('OPTION_ADDED').'</p></div>';
