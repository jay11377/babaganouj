...
class Thumbnail {
    ...
    function resize($maxWidth = 0, $maxHeight = 0) {
	 ...
	 /**
     * Resizes image to width x height without keeping the original ratio
     *
     * @param int $width
     * @param int $height
     */
	function resizeSkew($width, $height) {
		if(function_exists("ImageCreateTrueColor")) {
	...
	function resizeForCrop($maxWidth = 0, $maxHeight = 0) {
		if($this->currentDimensions['width']/$maxWidth < $this->currentDimensions['height']/$maxHeight)
			$this->resize($maxWidth,0);
		else
			$this->resize(0,$maxHeight);
	}
	
	/**
	 * Crops the image from calculated center in a rectagle of $width by $height
	 *
	 * @param int $width
	 * @param int $height
	 */
	function cropFromCenterSize($width,$height) {
	    //make sure the cropped area is not greater than the size of the image
		if($width > $this->currentDimensions['width']) $width = $this->currentDimensions['width'];
	    if($height > $this->currentDimensions['height']) $height = $this->currentDimensions['height'];
	    
		$startX = intval(($this->currentDimensions['width'] - $width) / 2);
	    $startY = intval(($this->currentDimensions['height'] - $height) / 2);
	...
	// I added this function to get the path
	function getThumbPath($imagepath, $suffix){
		$thumbpath = substr($imagepath, 0, strrpos($imagepath,'/'));
		$filebase = substr(basename($imagepath), 0, strrpos(basename($imagepath),'.'));
		$ext = substr(strrchr($imagepath, '.'), 1);
		$thumbname = $filebase."_".$suffix.".".$ext;
		$completethumbpath = '../'.$thumbpath.'/'.$thumbname;
		return($completethumbpath);
	}
	function getThumbPathDB($imagepath){
